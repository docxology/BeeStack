...
Every generated figure has a JSON sidecar. Curated manuscript figures also
carry caption text, alt text, intended section, Pandoc label, claim tier,
citation metadata where relevant, optional design-citation keys/source DOIs,
lightweight WCAG-style contrast checks, graphical-perception/provenance
scholarship for manuscript-facing evidence maps, and an unsupported-inference
boundary so a figure cannot silently overstate its evidentiary role.
...
- `output/llm/`: LLM-assisted research notes and external-analysis transcripts.
