...
comb grid with {{COMB_VOXELS}} voxels (default ${{COMB_SHAPE_X}} \times {{COMB_SHAPE_Y}} \times {{COMB_SHAPE_Z}}$),
...
({{FINAL_COMB_FRACTION}} at the end of the integrated run) and the
...
$[{{BROOD_TEMP_BAND_MIN_C}}, {{BROOD_TEMP_BAND_MAX_C}}]$ °C centred at {{BROOD_TEMP_TARGET_C}} °C
...
configured band, comb-voxel count ({{COMB_VOXELS}}), and forage-radius
