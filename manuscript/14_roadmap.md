...
tracks {{DIGITAL_TWIN_AXIS_COUNT}} axes and reports mean maturity
{{DIGITAL_TWIN_MEAN_MATURITY}}, with `population_twin_ready` resolved to
`{{DIGITAL_TWIN_READY}}`. The top blocker is: {{DIGITAL_TWIN_TOP_BLOCKER}}
The next named artifact is `{{DIGITAL_TWIN_NEXT_ARTIFACT}}`.
...
2. later, training surrogate agents from higher-fidelity rollouts so
   that the {{SWARM_AGENTS}}-to-{{REPRESENTED_COLONY_SIZE}} scale gap
