...
fields, and task allocation for a colony of {{SWARM_AGENTS}} simulated
agents representing {{REPRESENTED_COLONY_SIZE}} workers, and is
...
{{COMB_VOXELS}}-anchored size and drives them inward with wing-beat
...
{{METHODS_SWARM_CONTACT_PAIR_COUNT}} unique bee-contact pairs, which is
...
provenance, but extends the rollout to {{LONG_WAGGLE_ANIMATION_FRAMES}}
frames at {{LONG_WAGGLE_ANIMATION_FPS}} fps in the default
...
{{METHODS_SWARM_WAGGLE_ORIENTATION_ERROR_DEG}} degrees mean error,
{{METHODS_SWARM_WAGGLE_ORIENTATION_CONFIDENCE}} confidence, and a
waggle-phase coupling score of
{{METHODS_SWARM_WAGGLE_PHASE_COUPLING}}.
...
{{METHODS_SWARM_CONTACT_PAIR_COUNT}} unique bee-contact pairs, total
...
between the {{SWARM_AGENTS}} small-scene agent count and the
{{REPRESENTED_COLONY_SIZE}} BEEHAVE-scale represented count. Closing
