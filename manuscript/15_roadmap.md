...
research suite (§12) and the readiness review. It is ordered by
...
downstream improvements, while a large biomechanical calibration in
BeeBody only improves BeeBody.
...
completeness panel in §5.

*Acceptance criterion:* `CALCIUM_DATASET_COUNT >= 1` in the hydrated
manuscript variables and `BRAIN_DATA_PARSEABLE_FRACTION >= 0.7` for
the calcium modality.
...
This is the *least exciting* roadmap item and the one most likely to
yield long-term scientific value, because it preserves the
evidentiary trail through every other roadmap step.
