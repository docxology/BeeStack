...
It maintains a {{LATENT_DIM}}-dimensional belief state, temporal-polyethism
...
{{POLICY_HORIZON}} steps, and candidate expansion is bounded so
deterministic tests can cover every branch.

## Beliefs and caste

The `BeliefState` packs a {{LATENT_DIM}}-component latent vector, a
...
1. **Learned transition dynamics** — the current transition model is a
   small hand-coded forward simulator over the {{POLICY_HORIZON}}-step
